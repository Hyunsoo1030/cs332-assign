...
class Empty extends TweetSet {
  ...
  def descendingByRetweet: objsets.TweetList = Nil

  def mostRetweeted: objsets.Tweet = throw new NoSuchElementException("mostRetweeted on Empty")

  def union(that: objsets.TweetSet): objsets.TweetSet = that
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    ...
    val rest = remove(top).descendingByRetweet

    new Cons(top, rest)
  }

  def mostRetweeted: Tweet = {
    var left_most = left match{
      case _:Empty => elem
      case _ => left.mostRetweeted
    }
    var right_most = right match{
      case _:Empty => elem
      case _ => right.mostRetweeted
    }

    List(elem, left_most, right_most).maxBy(_.retweets)
  }
